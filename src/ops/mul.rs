...
#[js_function(3)]
pub fn mul(ctx: CallContext) -> Result<JsUndefined> {
  ...
  match first_array.typedarray_type {
    TypedArrayType::Int32 => {
      ...
      mul_with_cpu(first_array, second_array, result_array);
    }
...
#[inline]
fn mul_with_cpu<T: Mul + Copy>(first_array: &[T], second_array: &[T], result_array: &mut [T::Output]) {
...
mod tests {
  ...
  fn it_adds_slices_with_cpu() {
    ...
    mul_with_cpu(&first_array, &second_array, &mut result_array);

    assert_eq!(result_array, [1, 4, 9, 16, 25, 36, 49, 64, 81, 100]);
  }
}